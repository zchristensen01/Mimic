...
### The four documents, and how they fit

This guide is one of four layers, and each answers a question the others cannot.

| Document | Answers | When you read it |
|---|---|---|
| **[Glossary](Mimic_Glossary.md)** Appendix Zero-A | *What does that word mean?* | One block per session, on demand. Ordered by the session that first needs each term. |
| **Before You Build** Appendix Zero | *What am I looking at, and how do I set it up?* | Once at the start. |
...
**Where this guide fits into an evening:** each workbook session opens with a *Before you start* box, and where that box sends you here it names the interlude by title. The interludes are written to be read immediately before the session that needs them, not in one pass — [the reading order](README.md) says which goes with which.
...
*Before You Build* → "The Architecture, Plainly" sketched this in half a page. **This is the full version**, and the three things it adds are the ones that matter: the *rates* each layer owes the other, what the decision actually costs you, and how the seam is really built. If the sketch is still fresh, read the figure below carefully and skim the prose; if it is not, this chapter stands alone.
...
The specific bench, as ordered: two **12GA-N20** 12 V 100 RPM gearmotors with hall encoders and brackets, **TB6612FNG** drivers, a **12 V 5 A** supply, an **XL4015 CC/CV** module standing in for a bench supply (the current limit is the feature), an **AstroAI DM130B** multimeter, a **24 MHz 8-channel logic analyzer**, a **Quicko T12-952** soldering station, and a plywood board for a base. One **Arduino UNO R3** from the starter kit — an ATmega328P, checked
at the chip rather than taken from the box — runs the loop. You do not need the 3D printer, a Raspberry Pi, the EMG hardware, or a bench power supply to start.
...
#### One host, native Ubuntu — and why that is not just tidiness

The host is a laptop running **Ubuntu 24.04 natively**, and everything in this project happens on it: editing, flashing, serial monitoring, plotting, ROS 2, Gazebo and socketCAN. There is no second operating system anywhere in the plan, and that is a decision rather than an accident of what hardware you happened to own.

**Three of the things this project needs are kernel and driver features rather than applications.** `socketCAN` lives in the kernel — `candump` and `cansend` have nothing to attach to unless the module is there, which makes it a hard requirement the moment you put a CANable on the bus. Gazebo Harmonic needs real GPU access through a real graphics stack. ROS 2's DDS discovery needs a real network stack with no boundary in the middle, and a boundary is a well-known source of nodes that cannot see each other. Deterministic-ish timing on the host wants the same thing.

> **WATCH OUT** — The value of a single native host is diagnostic, not aesthetic. **A virtualisation layer is a second thing that can be wrong, and from inside a failure you cannot tell which of the two it was.** A board that does not enumerate, a simulator that renders black, a node that cannot find its peer — each of those has a mundane native-Linux explanation you can learn once and own, and each becomes unattributable the moment there is a layer under it. That is the two-fronts problem this guide keeps warning about, in its most expensive form: you debug the infrastructure while believing you are debugging the robot. If you draft code on another machine because the screen is bigger, sync through GitHub rather than a shared filesystem, and treat the Ubuntu machine as the machine of record — the one that builds, flashes and measures.

### The software you should install now

- **Ubuntu 24.04 LTS**, natively. This is the host and the bench machine both; everything below installs on it.
- **ROS 2 Jazzy Jalisco**, an LTS release targeting Ubuntu 24.04 and supported to May 2029. It is **not** the newest LTS — Lyrical Luth arrived in May 2026 — and staying one release back is a deliberate choice you should make rather than inherit. *Before You Build*, Part IV → "The ROS 2 Stack" carries the reasoning and the ADR it asks you to write. Install it early even though you will not use it until Stage 2 — having it working removes an obstacle from a stage that has plenty already.
...
- **Git**, and a GitHub account. Also **Git LFS** — note that `apt install git-lfs` and `git lfs install` are two separate acts, and only the second one activates the filters `.gitattributes` depends on.
...
- **PulseView / sigrok** for the logic analyzer — plus the **`sigrok-firmware-fx2lafw`** package, which holds the firmware your FX2-based analyzer is given at plug-in time. It has no flash of its own. Without that package the device enumerates, PulseView still cannot use it, and it looks like a faulty purchase. Budget an evening for udev rules and group membership generally: on Linux a USB device your user cannot open is the commonest way a working board looks broken.
...
Nothing in Mimic runs above 24 V, so the electrical risk to you is negligible. **The risks that are real are mechanical and thermal**, and there are seven of them: pinch injuries from a geared motor that has far more torque than you expect, runaway motion from a sign error, eye protection you do not currently own, electrode isolation in Stage 4, your motor's stall current against the TB6612's rating, soldering fumes, and leaving the rig powered and unattended with integral action enabled.

**[*Before You Build* → Safety Baseline](Mimic_Before_You_Build.md#safety-baseline) is the canonical list**, with each of the seven stated properly and the numbers attached. It is one page, it is in the document you read first, and repeating it here in a shorter form would only create a second version to disagree with it. Read it there.
...
> **WATCH OUT** — `socketCAN` is a kernel subsystem, not an application, and `candump` and `cansend` are a thin userspace layer over it. That is why the host for this project is a native Ubuntu machine rather than an environment running inside something else: where the module is absent the tools do not misbehave, they have nothing to attach to. Confirm it works on a virtual bus — `modprobe vcan`, then `candump` and `cansend` against `vcan0` — before you order the analyzer, not after.
...
- **PulseView / sigrok documentation**, plus the `sigrok-firmware-fx2lafw` package and the udev rules that let you open the device as your own user.
...
**The glossary for this document set is its own volume: [the Glossary](Mimic_Glossary.md).** It is
the canonical list: every term the Guide, the workbooks and the parts documents assume — mechanics,
C and C++ on a small chip, RTOS vocabulary, power electronics, the STM32's clock and interrupt
hardware, ROS 2, printing materials, and the camera and biosignal terms that arrive in Stages 3
and 4.

**It is ordered by the session that first needs each term**, not alphabetically and not by subject,
so that you read six to ten entries on the evening they matter rather than 174 in one sitting. Each
workbook session names its block by title in its *Before you start* box.
