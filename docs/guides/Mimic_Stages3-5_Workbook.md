...
## Before you start this volume

**Do not read this before the Stage 2 exit gate.** It is here so the arc is visible, not because any
of it is due.

**Every milestone below carries a *Before you start* box**, the same as a Stage 0 session does — what
to read, what is on the bench, and what done looks like. They are the navigation; the sessions
themselves are still yours to write when you get here. When you do arrive:

| Read | Where | For |
|---|---|---|
| **Glossary → Stage 3** | [Glossary](Mimic_Glossary.md#stage-3--eyes-and-voice) | 9 camera and pose terms |
| **Glossary → Stage 4** | [Glossary](Mimic_Glossary.md#stage-4--the-biomech-payload) | 11 signal, electrode and classifier terms |
| **Glossary → Stage 5** | [Glossary](Mimic_Glossary.md#stage-5--impedance-and-rehab-mode) | 3 force terms |
| **Interlude F · Frames of Reference** | [Project Guide](Mimic_Project_Guide.md#interlude-f--frames-of-reference) | Stage 3. Hand-eye calibration is a frames problem before it is a vision one |

Twenty-three terms, and none of them is needed before Stage 3 begins.
...
> **BEFORE YOU START** — About 30 minutes. **The first four camera terms, and they are all geometry.**
>
> - **[Glossary → Stage 3](Mimic_Glossary.md#stage-3--eyes-and-voice)** — read *Intrinsic matrix*, *Extrinsics* and
>   *Reprojection error* now; leave the rest until M3.2.
> - **[Project Guide → Interlude F, Frames of Reference](Mimic_Project_Guide.md#interlude-f--frames-of-reference)** — the
>   camera is another frame in the tree you built in Stage 2, not a new kind of thing.
>
> **On the bench:** the camera, rigidly mounted, and a printed calibration board on something flat.
>
> **Done looks like:** an intrinsic matrix and distortion coefficients **with a reprojection error you
> can quote as a number**, committed, and no reason to touch the calibration again unless the optics
> change. A calibration you cannot put an error figure on has not been checked.
...
**What this is for:** the calibration script will hand you a matrix and four or five coefficients, and
without this page they are magic numbers you paste into a config file. With it, you can say what each
entry means and why a number came out wrong.

**Why it matters beyond this milestone:** every later camera step — pose recovery in M3.2, hand-eye in
M3.3 — is this model used in reverse. A calibration you do not understand produces poses that are
confidently wrong, which is considerably worse than none.
...
>
> - **[Glossary → Stage 3](Mimic_Glossary.md#stage-3--eyes-and-voice)** — *Fiducial, and ArUco*, *PnP*, and
>   *Quaternion*.
> - **Your M3.1 calibration.** PnP needs the intrinsics; a wrong intrinsic matrix produces poses that
>   are confidently wrong, which is worse than none.
>
> **On the bench:** the calibrated camera and a printed marker whose size you measured rather than
> assumed.
>
> **Done looks like:** a marker's full six-DOF pose recovered from one image and **published as a tf2
> frame**, so the arm can be told where something is in coordinates it already understands. Not a
> detection overlay — a frame in the tree.
...
> **BEFORE YOU START** — About 35 minutes. **The hardest geometry in the project.**
>
> - **[Glossary → Stage 3](Mimic_Glossary.md#stage-3--eyes-and-voice)** — *Hand-eye calibration*, and re-read
>   *Extrinsics*.
> - **[Project Guide → Interlude F](Mimic_Project_Guide.md#interlude-f--frames-of-reference)** — again, and properly. The
>   whole milestone is one unknown arrow in a ring of known ones, and if the ring is not clear in your
>   head the equation is just letters.
>
> **On the bench:** the arm and the camera, both rigidly fixed relative to each other.
>
> **Done looks like:** the fixed transform from `camera_link` to `base_link` **solved rather than
> measured with a ruler**, and a before/after number showing what solving it bought you. The number is
> the deliverable; anyone can produce a transform.
...
> **BEFORE YOU START** — About 20 minutes. **No new vocabulary** — this is M3.1 to M3.3 assembled.
>
> - **Your Stage 2 S2.13 MoveIt work** and **M3.2's pose publishing.** This milestone is the join
>   between them, and nothing new is introduced.
> - **Write the failure list before the happy path.** The milestone asks for this explicitly and it is
>   the actual skill being taught.
>
> **On the bench:** the whole arm, the camera, and something to pick up.
>
> **Done looks like:** detect, plan, approach, grasp, lift, place — **and a named, tested behaviour for
> each of the three ways it can fail.** A pipeline that only has a happy path is a demo, not a system.
...
> **BEFORE YOU START** — About 15 minutes. **Check the current documentation rather than this page**
> for anything version-specific: the speech landscape moves faster than this workbook.
>
> - **No glossary block.** Nothing here is vocabulary you are missing; it is measurement discipline.
>
> **On the bench:** a microphone, in the room you actually work in, with **the network cable unplugged**
> — that is part of the deliverable, not a precaution.
>
> **Done looks like:** reliable text out of a microphone, locally, **with a word error rate you measured
> in your own room** rather than a demo that worked once. Nothing interprets the text yet — that is
> M3.6, deliberately kept separate.
...
> **BEFORE YOU START** — About 15 minutes. **The safety-relevant milestone of Stage 3.**
>
> - **No glossary block.** The idea you need is validation, and the milestone argues it in full.
> - **Your `docs/protocol.md`** from Stage 1 as a model: a schema with bounds is the same discipline as
>   a message specification with field widths.
>
> **On the bench:** the machine. Nothing moves during this milestone.
>
> **Done looks like:** a **structured, validated** command — action name plus bounded parameters — out
> of a transcript, **with malformed output rejected rather than repaired.** Repairing a model's bad
> output is how something unbounded reaches something that moves.
...
> **BEFORE YOU START** — About 10 minutes. The shortest milestone in the stage.
>
> - **No glossary block.**
>
> **On the bench:** a speaker.
>
> **Done looks like:** the arm confirms what it is doing and **says why when it refuses**, out loud,
> from a fixed set of phrases held as data rather than scattered through the code. The refusal is the
> part that matters: in assistive technology, a system that explains why it will not do something is
> worth more than one that confirms what it will.
...
> **BEFORE YOU START** — About 40 minutes. **Start this months early** — it needs no hardware and no
> electrodes, and it is the half of Stage 4 that can run in parallel with Stage 2 or 3.
>
> - **[Glossary → Stage 4](Mimic_Glossary.md#stage-4--the-biomech-payload)** — *sEMG*, *Feature window, RMS, MAV*,
>   *Hudgins set*, *Median frequency*, and *Confusion matrix, and cross-validation*. Five terms, and
>   the last one is the one that stops you fooling yourself.
>
> **On the bench:** nothing. A public dataset and Python.
>
> **Done looks like:** a feature-extraction and classification pipeline on public data, **with an
> accuracy figure you believe and a cross-subject figure you can explain.** If your accuracy looks
> excellent, suspect your split before you celebrate — overlapping windows leak, and the leak is
> invisible in the metric.
...
**Five functions, two or three lines each, and the docstring says why you are writing them by hand.**
The short version: you implement these again on a microcontroller in M4.3, where there is no NumPy and
no feature library, and a function you wrote once is a function you can port.

**Write them against the formulas in the comments, then check each one on a signal you already know
the answer for** — a constant, a pure sine — before you trust it on EMG. A feature extractor with a
sign error still produces plausible-looking numbers.
...
> **BEFORE YOU START** — About 45 minutes, and **this is the one milestone in the project with a
> physical safety requirement. Do not shorten the reading.**
>
> - **[Glossary → Stage 4](Mimic_Glossary.md#stage-4--the-biomech-payload)** — ***Galvanic isolation* first and
>   properly**, then *Ag/AgCl electrode*, *ADC*, and *Band-pass, rectify, envelope*.
> - **[Before You Build → Safety Baseline](Mimic_Before_You_Build.md#safety-baseline)** — the electrode clause specifically.
>
> **On the bench:** electrodes, the front end, and **either a USB isolator or a battery-powered
> microcontroller — both is better.** Nothing connects to you until the isolation is drawn and checked.
>
> **Done looks like:** your own EMG on screen from a correctly isolated bench, **with the isolation
> topology drawn and checked in three passes before anything was switched on.** The drawing is not
> documentation of the work; it is part of it.
...
> **BEFORE YOU START** — About 15 minutes. **No new vocabulary** — M4.1 and M4.2 supplied it.
>
> - **Your Stage 2 S2.10 gripper interface.** This milestone drives `open(fraction)`, which is why that
>   session made it a fraction rather than a pair of states.
>
> **On the bench:** the isolated EMG rig and the gripper.
>
> **Done looks like:** your muscle opening and closing the gripper — **and a fatigue plot of your own
> showing why a fixed threshold stops working after a few minutes.** Feeling that is the milestone;
> the control is almost incidental.
...
>
> - **Re-read *Confusion matrix, and cross-validation*** in the [Stage 4 block](Mimic_Glossary.md#stage-4--the-biomech-payload).
>   M4.1 taught you to split by subject; this milestone is about splitting by **session**, which is a
>   different and harsher test.
> - **Your M4.1 pipeline**, which you are now pointing at your own recordings.
>
> **On the bench:** the EMG rig, and recordings taken on **more than one day**. That constraint is the
> milestone — you cannot do this in one evening, so plan for it.
>
> **Done looks like:** a gesture classifier trained on your own recordings **that still works
> tomorrow**, with within-session and cross-session accuracy reported separately. The gap between those
> two numbers is the interesting result.
...
>
> - **[Glossary → Stage 4](Mimic_Glossary.md#stage-4--the-biomech-payload)** — *Complementary filter*, *Kalman
>   filter*, and re-read *Quaternion* from the [Stage 3 block](Mimic_Glossary.md#stage-3--eyes-and-voice).
> - **Your Stage 0 S10 work on derivative filtering.** A complementary filter is the same idea — trust
>   one signal at high frequency and another at low — and noticing that is the point.
>
> **On the bench:** two six-axis IMUs.
>
> **Done looks like:** shoulder and elbow angles from raw IMU data, by **a complementary filter and
> then a Kalman filter you wrote**, compared on the same recording. Two estimators on one dataset is
> the comparison; one estimator is a demo.
...
**What this is for:** to combine two sensors that are each wrong in a different way. A gyroscope is
accurate moment to moment but drifts over seconds; an accelerometer does not drift but is useless
while the limb is accelerating. Neither alone gives you an angle you can use.

**Why this particular equation:** the two coefficients sum to one, which makes it a high-pass on the
gyro plus a low-pass on the accelerometer, summing to unity gain. That is the whole trick, and it is
the same idea as the derivative filtering you did in Stage 0's S10.

**What you walk away with:** a working angle estimate and one tuning parameter, `α`, whose meaning you
can state — and a baseline to compare the Kalman filter against later in this milestone.
...
> **BEFORE YOU START** — About 15 minutes. **No new vocabulary.** The Stage 4 exit gate.
>
> - **Your M4.4 classifier and M4.5 filters**, and the end-to-end latency budget you are about to
>   measure against. Decide the budget before you measure, or the number will simply be whatever it is.
>
> **On the bench:** everything — IMUs, electrodes, the arm.
>
> **Done looks like:** the arm following your arm, posture from the IMUs and grip from your muscles,
> **without a keyboard** — and its end-to-end latency quoted as a measured number against a budget you
> stated in advance.
...
> **BEFORE YOU START** — About 30 minutes. **Optional stage, and the highest-value optional one.**
>
> - **[Glossary → Stage 5](Mimic_Glossary.md#stage-5--impedance-and-rehab-mode)** — 3 terms, and they are the whole
>   stage: an arm that is compliant rather than stiff has to command **torque**, which needs a motor
>   constant and a term that is not feedback at all.
> - **[Glossary → *Backdrivable*](Mimic_Glossary.md#before-s1--motor-forensics-and-bolting-it-down)** — from the
>   Before-S1 block, and read the caution in this stage about it. **A high-ratio gearbox may make this
>   stage physically impossible with your current motors**, and knowing that before you start is worth
>   an evening.
> - **Your Stage 0 PID work**, because the current loop below sits underneath the position loop you
>   already wrote, and nested loops are where people lose the thread.
>
> **On the bench:** the arm, plus current sensing you may not have yet.
>
> **Done looks like:** an arm you can push out of the way that pushes back proportionally — and can be
> told how hard to push back. Everything below is the order to build it in.
...
The other three workbooks carry a session index with hours. This one carries a **milestone** index,
because the sessions do not exist yet — you write them at the Stage 2 exit gate, and these are the
units of work they will be cut from. Hours are rough for the same reason.

| # | Milestone | Hours | Blocking on |
|---|---|---|---|
| M3.1 | Camera, calibrated | 4 | Camera |
| M3.2 | ArUco markers, pose to tf2 | 6 | M3.1 |
| M3.3 | Hand-eye calibration | 8 | M3.2, a working arm |
| M3.4 | Pick and place, closed loop | 12 | M3.3 |
| M3.5 | Local speech to text | 6 | Microphone |
| M3.6 | Language to intent | 8 | M3.5 |
| M3.7 | Speak back | 3 | M3.6 |
| | **Stage 3 total** | **~47 h** | 3–4 weeks at your pace |
| M4.1 | Dataset work, no hardware | 12 | **Nothing — start it early** |
| M4.2 | First real signal, safely | 6 | EMG front end, isolation |
| M4.3 | Threshold control | 5 | M4.2, S2.10's gripper |
| M4.4 | Your own classifier | 12 | Recordings on **more than one day** |
| M4.5 | Wearable IMUs and fusion | 10 | Two IMUs |
| M4.6 | Mirroring | 8 | M4.4, M4.5 |
| | **Stage 4 total** | **~53 h** | 4–5 weeks at your pace |
| S5 | Impedance, all four parts | ~30 | Current sensing. **Optional** |

Those two totals match Project Guide Appendix G. **M4.1 is the one to move**: it needs no hardware at
all and can run in parallel with Stage 2 or Stage 3, which is why the milestone says to start it
months early.

**There is no Stage 5 exit gate**, and that is deliberate rather than an omission — Stage 5 is
open-ended and optional, so it ends when you decide it does. If you want a gate, the honest one is
the *Prove it* at the end of the stage: an arm you can push out of the way that pushes back by an
amount you set.
...
Less prescriptive than the other volumes, for the same reason the sessions are: you will know what is
worth showing once you know what you built. Three things are worth planning for regardless, because
they are the ones that will not re-stage later.

- **Stage 3: the pick, filmed once, in one continuous shot.** Detection to grasp to place, uncut. A
  cut in a robotics clip reads as a hidden retry, whether or not it was one.
- **Stage 4: the fatigue plot from M4.3, and the cross-session accuracy gap from M4.4.** Both are
  results that surprised you, and both are far more interesting than a working demo of either.
- **Stage 5: a hand pushing the arm aside, and the same push at a higher stiffness.** Two clips of
  the same gesture is the entire idea of impedance control, shown rather than explained.

**And the electrode session's isolation drawing.** It is the one artifact in this project that is
about being careful rather than about being clever, and that reads well.
