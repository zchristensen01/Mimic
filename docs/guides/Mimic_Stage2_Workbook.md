...
## Before you start this stage

**You arrive here with Stage 1 tagged `v0.1-stage1-exit`,** and with the host stack installed and
proven — S2.0 is that proof, and it is the first session for a reason.

| Read | Where | With which sessions |
|---|---|---|
| **Glossary** — one block per session, named in each session's box | [Glossary](Mimic_Glossary.md) | S2.1 onward. Thirty-five terms across the stage, never more than eight at once |
| **Interlude F · Frames of Reference** | [Project Guide](Mimic_Project_Guide.md#interlude-f--frames-of-reference) | S2.2 and everything after. The single most useful page for this stage |
| **Stage 2 → Kinematics you derive yourself** | [Project Guide](Mimic_Project_Guide.md#kinematics-you-derive-yourself) | S2.2–S2.3, before you open CAD |
| **Before You Build → reading a datasheet** | [Before You Build](Mimic_Before_You_Build.md#the-skill-nobody-teaches-reading-a-datasheet) | S2.4, if motor selection sends you back to datasheets |

**Thirty-five terms, in three clusters that do not overlap** — the ROS 2 words (S2.1), the mechanics
words (S2.2–S2.4), and the printing words (S2.5 and S2.9). Reading all three at once is how Stage 2
starts feeling impossible on the first evening, so do not: **every session below carries its own
*Before you start* box** naming exactly the block it needs, and none of them is more than eight
terms.
...
Stage 1's exit criteria, all nine. Plus **the full host stack installed and proven** on the native
Ubuntu 24.04 machine — ROS 2 Jazzy, Gazebo Harmonic, colcon and `can-utils`. The machine itself has
been right since day one; what this gate adds is that all four have been run at least once and seen to
work. Meeting a broken ROS 2 install for the first time on the evening you also meet inverse
kinematics is the two-fronts problem in its purest form.
...
**You will be able to:** say which layer each of the four checks actually exercises — kernel, graphics path, network stack, userspace — as a property of the machine rather than as a rule you were given, and prove each one works on a host you set up yourself.

> **BEFORE YOU START** — About 20 minutes. **An install session, and the only one in Stage 2.**
>
> - **[Before You Build → The ROS 2 Stack](Mimic_Before_You_Build.md#the-ros-2-stack)** — the install order and the Jazzy
>   decision, including the ADR it asks you to write. Tonight executes what that section describes.
> - **No glossary block.** The ROS 2 vocabulary is S2.1's, deliberately: tonight you are proving four
>   things run, not learning what they mean.
>
> **On the bench:** the Ubuntu machine. No robot hardware at all.
>
> **Done looks like:** four checks passing — two nodes finding each other, `rviz2` rendering, `gz sim`
> rendering, and a virtual CAN frame round-tripping through `candump` — **and a host-setup ADR written
> the same evening, with the date on it.** The date is the load-bearing part.
...
3. `python3-colcon-common-extensions`, `python3-rosdep`, `python3-vcstool`
4. `can-utils` — `candump` and `cansend`. A thin userspace layer over a kernel subsystem, which is the
   practical reason this host is a machine rather than an environment inside one.
5. `sudo usermod -aG dialout $USER`, then log out and back in. Already done if you set the board up
   for PlatformIO in Stage 0; confirm rather than assume.

**Open a host-setup ADR in `docs/decisions/` and finish it the same evening**, with the date on it.
Which machine is the host and why that one, which ROS 2 and Gazebo releases you took and what you
turned down, and what broke on the way in. The date is the load-bearing part: it is the moment Stage 2
stopped being blocked on infrastructure, and it is the kind of thing that is obvious now and
...
- **talker and listener both run and neither sees the other.** Firewall, or two different
  `ROS_DOMAIN_ID` values, or one terminal sourced a workspace the other did not. Check `ros2 node
  list` from a third terminal — if it sees neither, the problem is the environment rather than the
  nodes.
- **`gz sim` opens a black window.** No GPU acceleration. On a laptop this is usually the graphics
  driver, or a hybrid-graphics setup rendering on the wrong device. It is a real problem with a real
  fix, and it is worth fixing now rather than in the session where you are also debugging a URDF.
...
> **CHECK YOURSELF** — Check 4 is the odd one out: it is the only one that touches the kernel rather
> than an application. Say **why**, in one sentence, and make it a sentence about what `modprobe vcan`
> actually did rather than about a rule you were given. Then answer the second half: checks 2 and 3
> exercise the graphics path and check 1 exercises the network stack. Name the layer each one would
> have caught, and which of the four you would run first next time something is wrong.
...
> **BEFORE YOU START** — About 40 minutes, and then **stop reading and run things.** None of this
> vocabulary survives being read about; it lands when you type the commands.
>
> - **[Glossary → Before S2.1](Mimic_Glossary.md#before-s21--ros-2-the-graph)** — 6 terms.
> - The official tutorials at `docs.ros.org`. **This is the one session where a tutorial is the work**
>   rather than a way of avoiding it, because the goal is fluency with somebody else's system.
>
> **On the bench:** nothing. Your robot does not appear in this session at all.
>
> **Done looks like:** you can take **a running ROS 2 system you did not write** and work out from the
> command line alone what it is doing — which nodes, which topics, which types, what QoS. And you can
> say when a topic, a service and an action are each right without reciting the definitions.
...
> **BEFORE YOU START** — About 45 minutes. **Paper session, and the first of the three that decide
> every mechanical choice in the stage.**
>
> - **[Glossary → Before S2.2](Mimic_Glossary.md#before-s22--forward-kinematics-derived)** — 3 terms.
> - **[Project Guide → Interlude F, Frames of Reference](Mimic_Project_Guide.md#interlude-f--frames-of-reference)** —
>   **the single most useful page for this stage.** Read all of it. A frame tree is what you are about
>   to derive, whether or not you are using tf2 yet.
> - **[Project Guide → Kinematics you derive yourself](Mimic_Project_Guide.md#kinematics-you-derive-yourself)** — why this
>   is derived rather than imported.
>
> **On the bench:** paper, and Python for the second half.
>
> **Done looks like:** given two joint angles you can compute where the gripper tip is — **on paper
> first, then in Python, and the two agree to within floating-point.** Plus your reachable workspace
> stated as two radii, with where each one comes from. Photograph the derivation.
...
> **BEFORE YOU START** — About 30 minutes. **Harder than S2.2, and the second paper evening.**
>
> - **[Glossary → Before S2.3](Mimic_Glossary.md#before-s23--inverse-kinematics-derived)** — 2 terms. *Singularity*
>   is not an edge case you can ignore; it is a place your arm will actually go.
> - **Your own S2.2 derivation**, open. Inverse kinematics is checked against forward kinematics —
>   round-trip a point through both and you should get back where you started.
>
> **On the bench:** paper and Python.
>
> **Done looks like:** given a target point you can compute the angles that reach it, **including
> deciding which of the two solutions you want and enforcing it.** Elbow-up and elbow-down both reach
> the same point; picking the wrong one on real hardware is how a link swings through a table.
...
**What this is for:** to go backwards — from a point you want the tip to reach, to the two joint
angles that put it there. Forward kinematics had one answer; this has **two**, and choosing between
them is half the session.

**Why the law of cosines and not something more general:** because a two-link planar arm is a
triangle, and a triangle with three known side lengths is exactly what that law solves. The
industrial answer is an iterative numerical solver; yours is closed-form, exact, and about six lines —
and knowing the closed form is what lets you check a solver later.

**What you walk away with:** `θ1` and `θ2` for any reachable point, a rule for which of the two
solutions you take, and a round-trip test — push a point through inverse then forward kinematics and
you should land back where you started.
...
> **BEFORE YOU START** — About 45 minutes. **This is the session that changes your parts list**, so do
> not start it on an evening you were hoping to order something.
>
> - **[Glossary → Before S2.4](Mimic_Glossary.md#before-s24--the-torque-budget)** — 8 terms.
> - **[Glossary → *Static, rated and stall torque*](Mimic_Glossary.md#before-s1--motor-forensics-and-bolting-it-down)** —
>   re-read this one from the Before-S1 block. **The whole budget turns on keeping those three apart**,
>   and your own motor's listing does not.
> - **Your `docs/motor-spec.md`** — the measured numbers from Stage 0's S1, which are the honest inputs.
>
> **On the bench:** paper, and your S2.2 geometry.
>
> **Done looks like:** a static torque budget per joint from a mass table you built, **and motors
> chosen on that evidence.** Expect the conclusion that your N20s cannot build this arm — that is not a
> failure of the session, it is the session. Defend the rejected options with the arithmetic that
> rejected them, and write the ADR.
...
> **BEFORE YOU START** — About 30 minutes, plus a CAD tutorial if you never did one.
>
> - **[Glossary → Before S2.5](Mimic_Glossary.md#before-s25--cad-and-design-for-printing)** — 4 terms. All four are
>   the difference between a model that looks right and a part that comes off the bed usable.
> - **Your S2.4 conclusions.** Link lengths and motor choice are inputs to this session, not outputs.
> - **Do one tutorial in your CAD tool first if you have not.** *Before You Build* told you to pick a
>   package and meet it early precisely so that tonight is one problem instead of two.
>
> **On the bench:** the computer. Nothing prints yet.
>
> **Done looks like:** two links, two joint housings and a rigid base, designed to be **assembled more
> than once** — and, the part people skip, **material density set on every part** so the model is a
> source of masses and inertias rather than only shapes. You need those masses in S2.7.
...
2. **Set the material density** on every part, in the CAD tool, to the filament you intend to print
   ...
   **Where this lives depends on your tool, and it is not always where you expect.** In FreeCAD it is
   not a modelling property at all — the mass-properties panel takes a density you supply, and the FEM
   workbench's material definitions are a separate mechanism. In Onshape it is a part property, or an
   assembly-level override. Find it in the tool you actually chose before you rely on the number, and
   **sanity-check one part by hand**: volume × density should match what the panel reports, and if it
   does not you have set density on the assembly rather than the body, which silently gives every part
   the same one.
...
>
> - **[Glossary → Before S2.6](Mimic_Glossary.md#before-s26--urdf-and-seeing-it-in-rviz2)** — 1 term, and it is the
>   file format the next four sessions all read.
> - **[Project Guide → Interlude F, Frames of Reference](Mimic_Project_Guide.md#interlude-f--frames-of-reference)** —
>   again. A URDF *is* a frame tree with a joint origin between each pair, and if the interlude has
>   gone fuzzy the XML will look arbitrary.
> - **Your S2.2 forward kinematics**, because that is what you check the URDF against.
>
> **On the bench:** the computer.
>
> **Done looks like:** your arm moving in `rviz2` from a slider — **and verified against your own
> kinematics rather than against how it looks.** Drive it to a known angle pair and confirm the tip
> lands where S2.2 says it should. "It looks about right" is how a wrong joint origin survives to S2.9.
...
>
> - **[Glossary → Before S2.7](Mimic_Glossary.md#before-s27--gazebo-harmonic)** — 1 term. The distinction between it
>   and URDF is the distinction between visualising and simulating.
> - **[Project Guide → Simulate before you solder](Mimic_Project_Guide.md#simulate-before-you-solder)** — why this session
>   comes before anything is printed.
>
> **On the bench:** the computer. **The masses and inertias from S2.5** — this is the session that
> needs them, and a missing inertia tensor is why arms explode on spawn.
>
> **Done looks like:** the arm moving under physics before a single part is printed. You should be able
> to say what a simulator needs that a visualiser does not — inertias, collision geometry, effort and
> velocity limits — **and recognise which one is missing from the way the arm misbehaves.**
...
> **BEFORE YOU START** — About 35 minutes.
>
> - **[Glossary → Before S2.8](Mimic_Glossary.md#before-s28--ros2control-with-mock-hardware)** — 3 terms, and the
>   first one has four words inside it the session depends on completely.
> - **[Project Guide → The seam](Mimic_Project_Guide.md#the-seam-how-the-two-layers-actually-talk)** — `ros2_control` is
>   that seam made concrete. Reading it first is the difference between a framework that seems
>   arbitrary and one that is obviously solving a problem you already have.
>
> **On the bench:** the computer. **Mock hardware deliberately** — no CAN, no motors.
>
> **Done looks like:** the full control stack running against a simulated robot, **and you can swap
> what is behind a live controller by changing one string.** That is the whole point: when S2.11
> introduces real hardware, the hardware is the only new variable.
...
> **BEFORE YOU START** — About 25 minutes. **Print a test coupon before you print a part.**
>
> - **[Glossary → Before S2.9](Mimic_Glossary.md#before-s29--print-and-assemble)** — 3 terms, about the machine
>   rather than the model. **Read them before you buy a printer, not after.**
> - Your slicer's documentation for the material you chose. Every filament in that glossary entry wants
>   different temperatures and a different nozzle.
>
> **On the bench:** the printer, the filament, calipers. **Calipers are not optional** — the whole
> session is measurement.
>
> **Done looks like:** a physical arm on a rigid base, **with every fit sized from a measured coupon
> rather than a nominal dimension** — and your machine's actual dimensional error written down as a
> number in `docs/tuning/`. Keep the parts that came out wrong.
...
> **BEFORE YOU START** — About 20 minutes. **No new vocabulary** — this session spends S2.4's.
>
> - **Your S2.4 torque budget**, open. The gripper's mass goes back into it tonight, and that feedback
>   loop is the point: a part you added changes a number you already computed.
> - **[Glossary → *Static, rated and stall torque*](Mimic_Glossary.md#before-s1--motor-forensics-and-bolting-it-down)**
>   once more, because sizing a servo from a required grasp margin is the same mistake waiting to
>   happen in a smaller package.
>
> **On the bench:** the servo, the printed fingers, and something to grip.
>
> **Done looks like:** a two-finger gripper that opens and closes on command, **reports whether it
> actually closed on something**, and whose real measured mass has been fed back into S2.4. You should
> be able to say why an open-loop servo cannot tell you whether it gripped anything, and name three
...
> **BEFORE YOU START** — About 25 minutes. **No new vocabulary** — S2.8 gave you all of it.
>
> - **[Project Guide → The seam](Mimic_Project_Guide.md#the-seam-how-the-two-layers-actually-talk)** — **the third time
>   this project makes the argument and the first time you are the one implementing it.** Worth
>   re-reading for that reason alone.
> - **Your own `docs/protocol.md` from Stage 1's S1.7.** `read()` and `write()` speak your protocol;
>   if the document is vague anywhere, tonight is where it hurts.
>
> **On the bench:** the arm, the CAN bus, both joints running Stage 1 firmware.
>
> **Done looks like:** a `SystemInterface` plugin whose `read()` pulls joint states off your bus and
> whose `write()` pushes targets out — **and neither of them blocks.** If either one waits on the bus,
> you have put a hard-real-time constraint inside a soft-real-time process, and the symptom will be
> intermittent and horrible.
...
**What this is for:** to work out what happens at the seam when the two sides of your robot disagree
about time — and they now definitely do. This is the third time the project has made this argument
(*Before You Build* Part I, the Project Guide's Chapter 2) and the first time you have to put numbers
...
> **BEFORE YOU START** — About 20 minutes.
>
> - **[Glossary → Before S2.12](Mimic_Glossary.md#before-s212--homing-and-calibration)** — 1 term, and
>   **[*Absolute versus incremental*](Mimic_Glossary.md#before-s1--motor-forensics-and-bolting-it-down)** in the
>   Before-S1 block is the other half of it. Read them together; the second explains why the first is
>   necessary at all.
> - **Your Stage 0 S11 limit-switch work** — the debounce you already solved once.
>
> **On the bench:** the arm, with limit switches fitted.
>
> **Done looks like:** an arm that knows where it is at power-up, **with the homing speed derived from
> switch bounce and loop rate** rather than picked because it looked safe. A homing routine that is too
> fast finds the switch and overshoots it, which is a crash rather than a calibration.
...
> **BEFORE YOU START** — About 30 minutes. **This is the moment the arm stops being joints.**
>
> - **[Glossary → Before S2.13](Mimic_Glossary.md#before-s213--moveit-2-and-cartesian-commands)** — 2 terms.
> - **Your own S2.3 inverse kinematics.** Tonight you hand the problem to a planner — and the reason
>   you derived it yourself is so that **when the planner and your arithmetic disagree you know which
>   to believe.** Have the Python open.
>
> **On the bench:** the whole arm, working.
>
> **Done looks like:** you command a **pose** rather than a set of angles, and coordinated multi-joint
> motion happens — and you have checked at least one planner solution against your own IK and can
...
> **BEFORE YOU START** — About 15 minutes. The last session of the stage.
>
> - **[Glossary → Before S2.14](Mimic_Glossary.md#before-s214--record-and-replay)** — 1 term.
> - **Your Stage 0 `tools/plot.py` and its four metrics.** Tonight computes the same four numbers from
>   a bag instead of a CSV, which is a claim about your instrumentation rather than about your robot.
>
> **On the bench:** the arm, and the Stage 2 exit gate open in front of you.
>
> **Done looks like:** `ros2 bag` capturing every joint state, command, gripper state and diagnostic;
> replayed; and **the same four step-response numbers you computed in Stage 0, computed again from the
> bag.** If they disagree with the CSV pipeline, one of your two instruments is lying and finding out
> which is the session. Then tag `v0.2-stage2-exit`.
...
**What this is for:** to find out, before you start recording everything by reflex, how many megabytes
a minute of robot costs. It is one multiplication, and the reason it is here is that bag files are the
one artifact in this project that can quietly fill a disk — and `.gitignore` already keeps them out of
the repository for exactly that reason.
...
## Repository additions this stage makes

Stage 2 adds more to the repository than the other two stages combined, and none of it is scaffolded.
The tree below is what should exist at `v0.2-stage2-exit`, with the session that creates each line.

```
ros2_ws/src/mimic_description/   # URDF/xacro, meshes          — S2.6
ros2_ws/src/mimic_bringup/       # launch files, controller yaml — S2.8
ros2_ws/src/mimic_hardware/      # the ros2_control plugin     — S2.11
ros2_ws/src/mimic_moveit_config/ # SRDF and MoveIt config      — S2.13
hardware/cad/                    # CAD sources AND STEP exports — S2.5. Git LFS
hardware/wiring/                 # dated photographs, as always
docs/derivations/                # kinematics and torque, photographed — S2.2 to S2.4
docs/decisions/                  # the busiest stage for ADRs
tools/kinematics.py              # your own FK/IK, used to check MoveIt — S2.2, S2.3
```

Four things worth deciding deliberately rather than by drift:

- **`build/`, `install/` and `log/` are already in `.gitignore`.** Colcon regenerates them and they
  are large and machine-specific. Check that they never appear in a `git status` you are about to
  commit.
- **CAD goes to Git LFS, and the STEP export goes with the source.** `.gitattributes` already routes
  both. Commit the export as well as the native file — a `.FCStd` nobody can open is not a portfolio
  artifact, and STEP is the format that outlives your choice of tool.
- **`tools/kinematics.py` is yours, not MoveIt's.** It exists so that S2.13 can check a planner
  against arithmetic you derived. Keep it even after MoveIt works; it is the evidence you understood
  the problem rather than configured a package.
- **Whether `ros2_ws/` lives in this repository at all is a real decision.** In it, everything is in
  one place and a clone is the whole robot; separate, the ROS 2 workspace can be built by colcon
  without the firmware tree confusing it. Most people put it in. **Write the ADR either way** — this
  is exactly the kind of choice that is obvious now and unreconstructable in eight months.
...
**S2.4 is two or three evenings and is written as one session.** It is the longest session in the
whole set — a mass table, a static budget, an acceleration term, a supply check, a belt-reduction
analysis and a motor decision with an ADR at the end of it. Splitting it across evenings is expected;
what is not expected is starting it on the evening you were hoping to place an order.
