...
> `candump` and `cansend` on the host, and it is the comfortable path. The host is already the right
> kind of machine for it, so the cost is the C$50 and an `apt install can-utils`, not an operating
> system. **The trade is what you learn**: buy it and you get a known-good view of the bus from the
> first frame, which makes every later fault obviously yours; use an ESP32-S3 with a transceiver as
> the monitor instead and you write the decoder yourself, which is more work and teaches you more
> about what is actually on the wire. Both are legitimate. Pick deliberately and write an ADR.
...
| ESP32-S3 cable | **USB-C.** Your UNO R3 is USB-B. |
