...
**You arrive here with Stage 0 tagged `v0.0-stage0-exit`.** If it is not tagged, the exit gate is not
met, and Stage 1 assumes a working `Joint` class you can port rather than a sketch you can read.

**Reading, in three groups, and take them when the session needs them rather than all at once:**

| Read | Where | With which sessions |
|---|---|---|
| **Glossary** — one block per session, named in each session's box | [Glossary](Mimic_Glossary.md) | S1.0 onward. Thirty-four terms across the stage, never more than thirteen at once |
| **Interlude D · Peeling Back Arduino** — the ladder | [Project Guide](Mimic_Project_Guide.md#interlude-d--peeling-back-arduino) | S1.0–S1.2. This stage is rungs 4 and 5 of it |
| **Interlude E · From Code to Silicon** | [Project Guide](Mimic_Project_Guide.md#interlude-e--from-code-to-silicon) | S1.0. Startup code, vector table, linker script |
| **Chapter 2 → The seam** | [Project Guide](Mimic_Project_Guide.md#the-seam-how-the-two-layers-actually-talk) | S1.5, when you design the protocol |

**The vocabulary load is heavier than Stage 0's, and it is lumpy rather than steady**: thirty-four
terms across ten sessions, but S1.5 alone carries nine and S1.6 carries thirteen. Those two evenings
need their reading done in advance more than any others in the project. The rest are one to three
terms each.

**Every session below has its own *Before you start* box**, exactly as Stage 0 does — what to read,
what to have on the bench, and what done looks like. The table above is the stage-level view; the
per-session boxes are the ones to actually obey.
...
> `candump`, and `cansend`. It is the comfortable path and you will probably want it eventually. The
> host is already the right kind of machine for it — socketCAN is in your kernel — so the cost is the
> C$50 and the `can-utils` install, not an operating system. **The real trade is what you learn.**
> Buy it and you get a known-good reference view of the bus from the first frame; use the ESP32-S3
> monitor instead and you write the decoder yourself, which is more work and teaches you more about
> what is actually on the wire. Both are legitimate. Pick one deliberately and write it in
> `docs/decisions/`.
...
> **BEFORE YOU START** — About 40 minutes. **This is the session with the widest gap between what you
> know and what is on screen**, so the reading is not optional.
>
> - **[Glossary → Before S1.0](Mimic_Glossary.md#before-s10--cubeide-and-the-shape-of-a-generated-project)** —
>   3 terms. Together they answer "what was Arduino doing that I never saw?"
> - **[Project Guide → Interlude D, Peeling Back Arduino](Mimic_Project_Guide.md#interlude-d--peeling-back-arduino)** —
>   the ladder. **Stage 1 is rungs 4 and 5 of it.** Read it and you are climbing a ladder you already
>   understood the shape of; skip it and CubeIDE looks like it generated a thousand lines at random.
> - **[Project Guide → Interlude E, From Code to Silicon](Mimic_Project_Guide.md#interlude-e--from-code-to-silicon)** —
>   the pipeline from your `.c` file to bytes in flash. Every file you are about to open sits at one
>   named point on it.
>
> **On the bench:** the Nucleo-F446RE and a USB cable. STM32CubeIDE installed and past its first-run
> updates — if you did that in Stage 0's S0 as instructed, tonight starts on time.
>
> **Done looks like:** a generated project that builds, and **you can point at three files and say what
> each does** — the startup assembly that runs before `main()`, the linker script that decides where
> your code lives, and the `.ioc` that holds the peripheral configuration. No LED yet. That is S1.1.
...
>
> - **[Glossary → Before S1.1](Mimic_Glossary.md#before-s11--blink-and-the-breakpoint-that-is-the-actual-point)** —
>   1 term.
> - **[Project Guide → What a debugger actually does](Mimic_Project_Guide.md#what-a-debugger-actually-does)** — five
>   minutes, and it is the argument for why tonight matters. **The LED is not the deliverable.**
>
> **On the bench:** the Nucleo. Nothing else — the ST-Link is on the board.
>
> **Done looks like:** an LED blinking from generated code, and — **the actual point** — the processor
> halted on a breakpoint with a variable's value visible in a watch window, and the GPIO output
> register open beside it so you can see the bit that drives the pin. If you have only got the LED,
> the session is half done.
...
> **BEFORE YOU START** — About 45 minutes, and this is the one Stage 1 session where **the vocabulary
> is the content.** Read the three terms properly; the session is unfollowable otherwise.
>
> - **[Glossary → Before S1.2](Mimic_Glossary.md#before-s12--the-clock-tree-and-why-180-mhz-is-not-automatic)** —
>   3 terms.
> - The **RM0390 reference manual**, clock-tree chapter, open in a tab. You are not reading it cover to
>   cover; you are going to need one diagram and two constraint tables out of it, and the session says
>   which.
>
> **On the bench:** the Nucleo. This is mostly a paper session with a debugger attached.
>
> **Done looks like:** you can state **four numbers** — your system clock, APB1, APB2, and what your
> timers are actually counting at — **and say where each came from.** The last one is not the same as
> its bus clock, and knowing why is the session. Every timing number in the rest of Stage 1 descends
> from these four.
...
**What this is for:** to produce four numbers — SYSCLK, APB1, APB2, and what your timers actually
count at — from an 8 MHz reference and a page of constraints, rather than accepting whatever the
configuration tool offers. **Every timing figure in the rest of Stage 1 descends from these four**,
so a wrong one here does not announce itself; it shows up two sessions later as a control loop that
runs at 1.07 kHz.

**Why derive rather than let CubeMX solve it:** because the tool will happily hand you a valid tree
that is not the one you wanted, and you will have no way to tell.
...
> **BEFORE YOU START** — About 20 minutes.
>
> - **[Glossary → Before S1.3](Mimic_Glossary.md#before-s13--hardware-quadrature--the-encoder-that-costs-no-cpu)** —
>   1 term, and it is the source of the commonest off-by-one in timer configuration.
> - **Your own Stage 0 S3** — the decoder you wrote, and the transition table you derived. **Have the
>   measured counts-per-revolution from `docs/motor-spec.md` in front of you.** Tonight's whole check is
>   that a different chip, decoding in silicon, gets the same number.
>
> **On the bench:** the Nucleo and the motor. The analyzer if you want to compare captures.
>
> **Done looks like:** TIM2 in encoder mode counting your encoder with **no CPU cost at all** — your
> loop reads a register instead of running an interrupt — and the same counts per output revolution
> you measured in Stage 0, measured again here. Two chips agreeing on a physical constant is a much
...
**Compare this with your Stage 0 `Encoder` class before you write anything.** It is the same object
with the work removed: no ISR, no transition table, no `volatile` count — the timer holds the count
in hardware and this struct just reads a register.

**Two members are here for later and should stay unused tonight.** `offset` is set by homing in Stage
2; `sign` is decided empirically the first time the count goes the wrong way. Leaving them present
and inert is deliberate, so that Stage 2 adds no fields.
...
>
> - **[Glossary → Before S1.4](Mimic_Glossary.md#before-s14--the-control-loop-in-c-on-bare-metal)** — 2 terms about
>   how an interrupt actually reaches your code on this chip.
> - **[Project Guide → Interlude B, The Shape of Time](Mimic_Project_Guide.md#interlude-b--the-shape-of-time)** — you read
>   this before Stage 0's S5. Re-read the last part: the argument changes when the hardware, rather
>   than your code, is what remembers to run on time.
> - **Your Stage 0 `PIDController`**, and the gains you recorded in `docs/tuning/`. You are porting,
>   not rewriting.
>
> **On the bench:** the Nucleo, motor and driver. The analyzer on your loop-rate pin.
>
> **Done looks like:** your Stage 0 controller running from a **timer interrupt at 1 kHz** with
> identical behaviour — and the proof is a step response from the STM32 overlaid on the one from the
> UNO. Gains that transfer unchanged across two chips and two rates are evidence your units were
> right. Gains that do not are evidence they were not, which is equally useful and much more annoying.
...
**This is your Stage 0 `loop()` turned inside out.** There, your code asked "is it time yet?"; here
the hardware calls you and the only question left is which timer did it. That inversion is the whole
session, and the comment on the callback below is the trap it creates.

**`DT_S` is not a free parameter.** It must agree with the TIM6 arithmetic you just derived — if the
timer ticks at 1 kHz and this says `0.001f`, your integral and derivative terms are correct; if either
changes without the other, they are silently wrong and the controller merely feels badly tuned.
...
> **BEFORE YOU START** — About an hour, and **this is the heaviest reading block in the whole
> project.** Split it: five terms before the session, four when you reach the queue.
>
> - **[Glossary → Before S1.5](Mimic_Glossary.md#before-s15--freertos--two-tasks-and-the-demonstration-that-justifies-it)** —
>   9 terms. Read *RTOS*, *Task*, *Context switch*, *Priority inversion* and *Stack high-water mark*
>   first; leave *Mutex*, *Semaphore*, *Queue* and *DMA* until the session asks for them.
> - **[Project Guide → Interlude D, the ladder](Mimic_Project_Guide.md#the-ladder)** — where an RTOS sits relative to the
>   superloop you have been writing since Stage 0.
>
> **On the bench:** the Nucleo and the analyzer. **The analyzer is not optional tonight** — the whole
> deliverable is a capture.
>
> **Done looks like:** a 1 kHz control task and a slow telemetry task running together, **and an
> analyzer capture proving the slow one cannot damage the fast one.** The claim is not "it seems fine";
> it is a trace showing the control task's period unchanged while the telemetry task is deliberately
> made to misbehave. You should also be able to name four ways to move data between two tasks and
> defend the one you picked.
...
> **BEFORE YOU START** — About an hour. **Thirteen terms, and the second-heaviest evening in the
> stage.** They divide cleanly, which makes it manageable: five are about the wire, the rest about the
> protocol on it.
>
> - **[Glossary → Before S1.6](Mimic_Glossary.md#before-s16--can-physically--two-nodes-and-a-counter)** — 13 terms.
> - **[Project Guide → The four buses, side by side](Mimic_Project_Guide.md#the-four-buses-side-by-side)** — one figure.
>   `I²C` and `SPI` are in tonight's glossary block for contrast: knowing what CAN does that they do
>   not is most of understanding why it exists.
>
> **On the bench:** two boards, two transceivers, one twisted pair, two 120 Ω resistors, and a
> multimeter. **The meter matters here** — a silent bus is diagnosed with a resistance reading long
> before it is diagnosed with code.
>
> **Done looks like:** **a number arriving at the other end. Nothing else.** No protocol, no joint
> state, one counter. You should also be able to derive your bit timing from the bus clock rather than
> copying it, and read 60 Ω across the bus with the power off.
...
> **BEFORE YOU START** — About 30 minutes. **A paper session** — no board, no keyboard until the end.
>
> - **[Glossary → Before S1.7](Mimic_Glossary.md#before-s17--designing-the-joint-protocol--paper-before-keyboard)** —
>   2 terms. *Arbitration* is why an identifier is also a priority, and that single idea is what your
>   emergency-stop message depends on.
> - **[Project Guide → Designing your CAN protocol](Mimic_Project_Guide.md#designing-your-can-protocol)** — the reasoning,
>   the starting scheme, and the decisions it deliberately leaves to you. Read it fully; tonight is the
>   session it was written for.
>
> **On the bench:** paper. Genuinely.
>
> **Done looks like:** **`docs/protocol.md` written well enough to hand to someone else** — every field
> width justified, every rate stated, and the behaviour on a missed sequence number decided. Plus an
> ADR for at least one choice that had a real alternative, because most of tonight's decisions do.
...
> **BEFORE YOU START** — About 25 minutes. **No new vocabulary** — tonight spends what S1.6 and S1.7
> gave you.
>
> - **[Project Guide → The seam](Mimic_Project_Guide.md#the-seam-how-the-two-layers-actually-talk)** — the third time this
>   project makes the argument, and the first time you implement the joint's half of it.
> - **Your own `docs/protocol.md` from S1.7**, open. If it does not say what a joint does when commands
>   stop arriving, tonight is where that gap becomes obvious.
>
> **On the bench:** both boards on the bus from S1.6.
>
> **Done looks like:** command and state frames flowing to your own spec, a **hardware** filter doing
> the discarding rather than your code, and a joint that does something defensible when the host goes
> quiet — with the timeout **derived from your command rate**, not chosen because it felt safe. Being
> able to say why holding position is bounded where continuing a velocity is not is the point.
...
> **BEFORE YOU START** — About 10 minutes. The shortest session in the stage, and the exit gate.
>
> - **No new vocabulary.** Re-read your own S1.7 protocol document and the Stage 1 exit gate at the end
>   of this workbook before you begin, so you know what you are demonstrating.
>
> **On the bench:** both boards, the bus, both motors.
>
> **Done looks like:** two joints running **the same binary, byte for byte**, with different node IDs
> taken from somewhere outside the firmware — and you can prove the identity with a hash rather than
> assert it. One binary flashed twice is a much stronger claim than two binaries that both work, and
> the proof is the difference. Then tag `v0.1-stage1-exit`.
...
Unlike Stage 0's, **none of this is scaffolded** — Stage 1 is the first stage where you create the
directories as well as fill them. The tree below is what the repository should look like at
`v0.1-stage1-exit`, and each line names the session that creates it.

```
firmware/stage1-stm32/     # the CubeIDE project              — S1.0 creates it
  Core/Src/                #   your code, beside generated code
  *.ioc                    #   the CubeMX file. COMMITTED — S1.0 requires it
docs/protocol.md           # the CAN message specification    — S1.7 writes it. Exists as a stub
docs/decisions/            # by now this folder is real       — S1.6 or S1.7 at the latest
docs/tuning/               # the STM32 step responses         — S1.4, overlaid on Stage 0's
```

Three things worth knowing about this tree:

- **The `.ioc` is committed and it is not optional.** It is the only record of your peripheral
  configuration, and a CubeIDE project without it is a folder of generated C nobody can regenerate.
  `.gitattributes` already marks it as text for exactly this reason.
- **`firmware/stage0-uno/` does not go away.** Stage 0's rig stays buildable and stays in the
  history — it is the thing your Stage 1 step responses are compared against in S1.4, and a repository
  that quietly deletes its earlier work has thrown away the comparison that makes the later work
  meaningful.
- **CI does not build this project.** The GitHub Action runs `pio run` against the PlatformIO project
  only; a CubeIDE project needs a different toolchain in the runner. **That is a gap you can close if
  you want to** — it is a legitimate piece of work, it belongs in an ADR, and it is not required by
  the exit gate.
